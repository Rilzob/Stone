...
public class Parser {
    ...
    public Parser identifier(HashSet<String> reserved) { // 向语法规则中添加终结符(除保留字reserved外的标识符)
    ...
    public Parser token(String... pat) { // 向语法规则中添加终结符(与pat匹配的标识符)
        elements.add(new Leaf(pat));
        return this;
    }
    public Parser sep(String... pat) { // 向语法规则中添加未包含于抽象语法树的终结符(与pat匹配的标识符)
        elements.add(new Skip(pat));
        return this;
    }
    public Parser ast(Parser p) { // 向语法规则中添加非终结符p
        elements.add(new Tree(p));
        return this;
    }
    public Parser or(Parser... p) { // 向语法规则中添加若干个由or关系连接的非终结符p
        elements.add(new OrTree(p));
        return this;
    }
    public Parser maybe(Parser p) { // 向语法规则中添加可省略的非终结符p(如果省略，则作为一棵仅有根节点的抽象语法树处理)
        Parser p2 = new Parser(p);
        p2.reset();
        elements.add(new OrTree(new Parser[] { p, p2 }));
        return this;
    }
    public Parser option(Parser p) { // 向语法规则中添加可省略的非终结符p
        elements.add(new Repeat(p, true));
        return this;
    }
    public Parser repeat(Parser p) { // 向语法规则中添加至少重复出现0次的非终结符p
        elements.add(new Repeat(p, false));
        return this;
    }
    public Parser expression(Parser subexp, Operators operators) { // 向语法规则中添加双目运算表达式(p是因子，op是运算符表)
    ...
    public Parser insertChoice(Parser p) { // 为语法规则起始处的or添加新的分支选项
        Element e = elements.get(0);
